		...
		<div id="mainContentCss" class="container text-light">
			<div align="center" class="cards noticecontainer">
				...
				<section class="card noticecard">
					...
					<p class="cardblack">
						Semester:
						<%=Integer.parseInt(currentnotice.getSemester())==0?"ALL":currentnotice.getSemester()%>
						(branchcode:
						<%=String.format("%02d", Integer.parseInt(currentnotice.getBranch()))%>)
					</p>
					<p class="cardtitle">
						(<%=new SimpleDateFormat("dd-MM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(currentnotice.getStartDate()))%>
						to
						<%=new SimpleDateFormat("dd-MM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(currentnotice.getEndDate()))%>)
					</p>
					<%
                                ...
                                %>
					<p class="cardtitle">
						<a href='downloadfile'>Download Attached File</a>
					</p>
					<%
                                }
                            ...
                            %>
					<p>
						<button class="cardbutton delete_data_id_notice" data-id="<%=id%>">Delete</button>
					</p>
					<% } else { %>
					<p class="cardtitle">
						submitted by (<%=currentnotice.getProfessorCode()%>)
					</p>
					<%}%>
					<p>
				...
				<section class="card">
					<img class="card_img"
						src="${pageContext.request.contextPath}/allfiles/404.jpg"
						alt="404" style="width: 100%">
					<h2 class="cardblack">Awww...Don't cry</h2>
					<p class="cardtitle">It's just a 404 error</p>
					<p class="cardblack">What you are looking for may have been
						misplaced in Long term memory</p>
					<p>
						<button class="cardbutton"
							onclick="window.history.go(-1); return false;">Go Back</button>
					</p>
