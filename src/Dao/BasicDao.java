...
public class BasicDao {
    ...
    public static List<NoticeBean> getCurrentStudentNotice(Connection con, int collegecode, int semester, int branchcode) {
        ...
        try {
            ...
            rs = s.executeQuery("select * from tblnotice where collegecode='"+collegecode+"' and branchcode='" + branchcode +"'  and (semester='" + semester + "' or semester='0')  and DATE(end_date)>=DATE(NOW())  order by id desc");
            System.out.println("Please enable date filter....");
            while(rs.next()) {
                NoticeBean n = new NoticeBean();
                n.setId(rs.getLong(1));
                n.setTitle(rs.getString(2));
    ...
    public static NoticeBean previewSelectedNotice(Connection con, int noticeid,int collegecode, String logintype, int semester, int branchcode) {
        ...
        try {
            ...
            if(logintype.equals("student"))
                rs = s.executeQuery("select * from tblnotice where id='" + noticeid + "' and collegecode='"+collegecode+"'  and branchcode='" + branchcode +"' and (semester='" + semester + "' or semester='0') and DATE(end_date)>=DATE(NOW())");
            else
